...
if [ "$1" = "SPM" ]; then
  ...
  if [[ -n "$2" ]]; then
    ...
    CONFIG_DEBUG_FILE_PATH=${BUILT_PRODUCTS_DIR%/Build*}/SourcePackages/checkouts/oscaessentials-ios/OSCAEssentials/OSCAEssentials/Resources/$CONFIG_FILE_NAME_DEBUG
    # release file path ".../API_Release.plist"
    CONFIG_RELEASE_FILE_PATH=${BUILT_PRODUCTS_DIR%/Build*}/SourcePackages/checkouts/oscaessentials-ios/OSCAEssentials/OSCAEssentials/Resources/$CONFIG_FILE_NAME_RELEASE
  else
